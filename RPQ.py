...
def bfs(graph, NFA, start):
    ...
    return graphsolutions, visited, list(set(edgelist)), broadcasts #set of graph nodes in terminal nodes of product automaton; list of visited nodes; list of traversed edges; list of broadcast queries
...
def bfsEstimator(probabilities, graphsize, NFA):
    ...
    while queue:
        ...
        if vertex not in visited:
            ...
            for (vg2,outlabel) in generatedgraph.get(vgraph, []): #in case the above generated nothing (a probabilities issue)
        ...
        if (vertex[0],vertex[1]) not in visited:
            ...
            for label in (vautom.transitions.keys()): #these are the outgoing labels we're interested in
                ...
                if (label not in gengraphlabels.get(vgraph,[])): #no!
                    ...
                    whichones=set()                    
                    if (prevlabel==label):
                        howmany = np.random.binomial(graphsize-1,probabilities[prevlabel].get(label,0))+1 #binomial (n-1,p)+1 because we arrived with one edge, so we know at least one exists.
                        whichones.add(prevnode) #there's  link back to the node we came from; this one is known                    
                    else:
                        howmany = np.random.binomial(graphsize,probabilities[prevlabel].get(label,0)) #binomial (n,p)                                        
                    while len(whichones)<howmany:
                        whichones.add(np.random.randint(graphsize))
...
def build2GramGraphModel(gfilename, gsize= None): 
    ...
    for l1 in len2seqs.keys():
        ...
        for l2 in len2seqs[l1].keys():
            if (l1 == l2):#special case because there is always one edge back to the previous node, we want to count the probabilities of the others. (checking for neg values with 0, this happens in a sample)
...
def multisourceEstimation(gfile,qfile, gsize=None, outfile=None,reps =10):
    ...
    for line in qf:
        ...
        NFA= compile(regex) #only compile the regex once... not that it's super costly, but let's not do it thousands of times!
        nfas.append(NFA)    
    ...
    for repeat in range(reps): #repeat k times the full list of experiments.
        ...
        for NFA in nfas: #just iterate over the NFAS directly
            ...
            sol, vis, edgelist, bc = bfsEstimatorBigram(probabilities, gsize, NFA) #get: solutions, visited nodes, visited edges, broadcasts
            # --------write results to file ----------
            vnodes = set([v1 for (v1,v2) in vis])
            bc = sorted(['('+str(v)+','+a+ ')' for (v,a) in bc]) #list+stringify set of broadcasts
            totalcost = sum([len(re.findall('[1-9]+',message)) for message in bc]) #for each broadcast message, get all edges and nodes (numbers) and count them. Then sum over all messages.
    ...
    for line in qf:
        ...
        if (outfile):
            for i in range(len(sols)):
                outf.write("q"+str(cnt)+"\t"+str(sols[i])+"\t"+str(vnodes[i])+"\t"+str(elist[i])+"\t"+str(bc[i])+"\n")        
    ...
    if (strat == 'r') :
        if (dm== 'rdf'):
            for index in range(numberof):
                ...
                outfile = open(fnamebase+ "_"+ "{0:03d}".format(int(ratio*1000))+"_"+strat+ "_"+dm+ "_"+str(index), "w")
                for e in selected:
