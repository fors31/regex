...
class State:
    ...
    def getKeysToState(self, state):
        return [k for k in self.transitions.keys() if (self.transitions[k] == state)] + (
            ['EPS'] if (state in self.epsilon) else [])
...
class NFA:
    ...
    def deepCopy(self):
        ...
        for newstate in newstateDict.values():
            ...
            newstate.epsilon = list(map(lambda s: newstateDict[s.name],
                                        oldstateDict[newstate.name].epsilon))  # copy over the epsilon transitions
            newstate.transitions = {ks[0]: newstateDict[ks[1].name] for ks in oldstateDict[
                newstate.name].transitions.items()}  # copy over the epsilon transitions
    ...
    def uglyprint(self):
        ...
        while queue:
            vertex = queue.pop(0)
            print("v: " + vertex.name + (" [start]" if vertex == self.start else "") + (
                " [end]" if vertex.is_end else ""))
